...
type AssessmentType =
  | PredefinedAssessmentKey
  | "DAILY_MONITORING"
  | "FOSTER_UPDATE";

// --- Configuration Data (Now with TypeScript) ---
const PREDEFINED_ASSESSMENTS: Record<
  PredefinedAssessmentKey,
  PredefinedAssessmentConfig
> = {
  INTAKE_BEHAVIORAL: {
    ...
    fields: [
      {
        ...
        valueOptions: ["Quiet", "Anxious", "Barking", "Alert"],
      },
      {
        name: "Leash Manners",
        valueOptions: [
      ...
      },
      {
        ...
        valueOptions: ["Tolerant", "Hesitant", "Pulls Away", "Mouths Hand"],
      },
      {
        name: "Food Guarding (High Value)",
        valueOptions: ["None", "Stiffens", "Growls", "Snaps"],
      },
      {
        name: "Toy Possessiveness",
        valueOptions: ["Shares Easily", "Reluctant to Drop", "Guards Toy"],
      },
    ],
  },
  INTAKE_MEDICAL: {
...
export function AssessmentForm() {
  const [assessmentType, setAssessmentType] =
    useState<AssessmentType>("INTAKE_BEHAVIORAL");
  ...
  useEffect(() => {
    const config =
      PREDEFINED_ASSESSMENTS[assessmentType as PredefinedAssessmentKey];
  ...
  const currentConfig = PREDEFINED_ASSESSMENTS[
    assessmentType as PredefinedAssessmentKey
  ] || { allowCustomFields: true };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-2">
          ...
          <Select defaultValue="buddy">
            ...
            <SelectContent>
              ...
              <SelectItem value="whiskers">
                ID: 12346 - Whiskers (Cat)
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="space-y-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="INTAKE_BEHAVIORAL">
                Intake Behavioral (Strict)
              </SelectItem>
              <SelectItem value="INTAKE_MEDICAL">
                Intake Medical (Hybrid)
              </SelectItem>
              <SelectItem value="DAILY_MONITORING">
                Daily Monitoring (Flexible)
              </SelectItem>
              <SelectItem value="FOSTER_UPDATE">
                Foster Update (Flexible)
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="space-y-4">
        <h4 className="text-lg font-medium">Assessment Details</h4>
        {/* Grid Header */}
        ...
        <div className="space-y-4">
          {fields.map((field) => (
            <div
              key={field.id}
              className="grid grid-cols-1 md:grid-cols-[2fr_2fr_3fr_auto] gap-4 items-start"
            >
              {/* Field Name */}
              <div className="space-y-1">
                <Label htmlFor={`name-${field.id}`} className="md:hidden">
                  Field Name
                </Label>
                {field.isPredefined ? (
                  <p className="h-10 flex items-center px-3 text-sm font-medium text-muted-foreground">
                    {field.name}
                  </p>
                ) : (
                  <Input
              ...
              {/* Field Value */}
              <div className="space-y-1">
                <Label htmlFor={`value-${field.id}`} className="md:hidden">
                  Value
                </Label>
                {field.valueOptions ? (
                  <Select
                  ...
                  <Input
                    id={`value-${field.id}`}
                    placeholder={
                      field.placeholder || "e.g., Calm and disinterested"
                    }
                    value={field.value}
                    onChange={(e) =>
              ...
              {/* Field Notes */}
              <div className="space-y-1">
                <Label htmlFor={`notes-${field.id}`} className="md:hidden">
                  Notes
                </Label>
                <Input
                  id={`notes-${field.id}`}
